...
*/
console.log(MenuElemaniOlustur("Karisik Pizza", 5, "Pizzalar"));
...
degerlendirmeler[7].geribildirim =
  "bu mekan bir harika dostum, yinede garsonun gülüsememesinden puan kırdım";
...
function DegerlendirmeEkle(Sonuç, a, b, c) {
  let newObject = { isim: "a", puan: "b", gerbildiri: "c" };
  Sonuç.push(newObject);
  return Sonuç;
}
/*Kodlar buraya */
...
function SonDegerlendirmeyiAl(Liste, i) {
  return `${degerlendirmeler[i].isim} isimli kişi ${degerlendirmeler[i].puan} puan verdi ve şunları yazdı: ${degerlendirmeler[i].geribildirim}`;
}
